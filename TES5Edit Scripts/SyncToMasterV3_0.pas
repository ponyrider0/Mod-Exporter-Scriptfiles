...
var
	bDeleteSkipped, bSkipInjection, doOnce: boolean;
...
begin
	...
	try
		...
		clbOptions.Height := 40;
		clbOptions.Items.Add('Skip non-worldspace injections');
...
//===========================================================================
function FindRefByBestMatch(cell, e: IInterface; threshold: double; refMatchRequired, sigMatchRequired: short): IInterface;
var
...
//	cellChild := ChildGroup(cell);
	if (Assigned(cellChild)=0) then begin
		addmessage( Format( 'DEBUG: ERROR.  Could NOT find a cell in ChildGroup for cell %s.  How did this happen? Dont know.', [IntToHex(GetLoadOrderFormID(cell),8)]) );  
		exit;
	end;
	for i := 0 to Pred(ElementCount(cellchild)) do begin
		r := ElementByIndex(cellchild, i);
...
//			Result := r;
			if (true) then begin
				...
				addmessage(Format('DEBUG: exact match found: %s = %s, continue with injection...', [IntToHex(GetLoadOrderFormID(e),8), IntToHex(GetLoadOrderFormID(r),8)]));
				Result := r;
			end
			else begin
				// return nil to skip injection, then delete exact match
...
//			end;
	...
	if matchList.Count = 0 then begin
		addmessage(Format('DEBUG: No match found after processing %d records from masterCell %s.', [i, IntToHex(FormID(cell),8)]));
		matchlist.free;
		exit;
	end;
	...
	if (Result <> nil) then
		addmessage(Format('DEBUG: Exact match not found. Best match was %s = %4f', [IntToHex(GetLoadOrderFormID(Result),8), bestMatch]));
	matchList.free;
...
//		addmessage(Format('DEBUG: great, the targetCell %s is already injected into the master, so we can proceed to searching for an equivalent master record', [IntToHex(GetLoadOrderFormID(targetCell),8)]));
	...
	masterReference := FindRefByBestMatch(masterCell, e, 100, false, true);
...
begin
	...
	if (doOnce = false) then begin
		...
		if (HasMaster(targetFile, GetFileName(masterFile))) then begin
			...
			// Issue warning
			AddMessage( Format('WARNING: file %s is already linked to master %s.  This script was designed for unlinked files, results may not be correct.', [GetFileName(targetFile), GetFileName(masterFile)]) );
		end
		else begin
