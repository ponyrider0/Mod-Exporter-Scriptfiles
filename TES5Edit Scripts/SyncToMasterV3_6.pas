...
begin
    ...
    if Assigned(masterLand) then begin
        ...
        addmessage('masterLAND NOT found');
    end;
...
//===========================================================================
function FindRefByBestMatch(cell, e: IInterface; threshold: double; refMatchRequired, sigMatchRequired, overRideGroupType: short): IInterface;
var
    bExactSubstitution: boolean;
	originalRecord, cellChild, r: IInterface;
	returnVal: IInterface;
	i, numMatches, maxMatches, recordType, matchFailed: integer;
...
begin
    bExactSubstitution := false;
    if (refMatchRequired = 0) and (sigMatchRequired = 0) and (bRefExactCoords = true) then begin
        bExactSubstitution := true;
    end;
    ...
    if ((overRideGroupType = 1) and (GetIsPersistent(cell)=0)) then recordType := 9;
    if ((overRideGroupType = 2) and (GetIsPersistent(cell)=0)) then recordType := 10;
    ...
    for i := 0 to Pred(ElementCount(cellchild)) do begin
        ...
        if (originalRecordID = GetLoadOrderFormID(BaseRecord(r)) ) then begin
            ...
            // this is a work-around for condition where exactmatch is calculated without any actual matches
            numMatches := numMatches+0;
            maxMatches := maxMatches+10;
        end;
        // ignore scale if using exactsustituion
        if (bExactSubstitution = false) then begin
    ...
    //              AddMessage (Format('Scale match was found: mScale(%4f)-lScale(%4f)=dScale(%4f) < 0.50.', [mScale, lScale, abs(mScale-lScale)]) );
        ...
        // ignore other coords if bExactSubstitution is on
        if (bExactSubstitution = false) then begin
            // compare z-coord
	        if (dz < 1) then begin
                numMatches := numMatches+20;
            ...
            // if dx, dy, dz matches then give extra bonus points
            if (dx = 0) and (dy = 0) and (dz = 0) and (mX <> 0) and (mY <> 0) and (mZ <> 0) then begin
                numMatches := numMatches+20;
                maxMatches := maxMatches+20;
            end;
    ...
    //          numMatches := numMatches+0;
        ...
        if (bExactSubstitution = true) then begin
            // if in bExactSubstitution mode, use all or none matching on X,Y
            if (dx = 0) and (dy = 0) and (mX <> 0) and (mY <> 0) then begin
                matchRate := 1.0;
            end
            else begin
                matchRate := 0;
...
//              addmessage(Format('DEBUG: record (%s) was not a match -- matchscore was %f4, continuing...', [IntToHex(FormID(r),8), matchRate]));
    ...
    if matchList.Count = 0 then begin
        ...
        matchlist.free;
        // first fallback match
        if ((GetIsPersistent(e) <> 0) and (overRideGroupType = 0) and (GetIsPersistent(cell)=0)) then begin
            returnVal := FindRefByBestMatch(cell, e, threshold, refMatchRequired, sigMatchRequired, 1);
        end;
        // second fallback match
        if ((Assigned(returnVal) = false) and (overRideGroupType = 0)) then begin
            returnVal := FindRefByBestMatch(cell, e, threshold, refMatchRequired, sigMatchRequired, 2);
        end;
        Result := returnVal;
        exit;
    end;
...
//      addmessage(Format('DEBUG: looking for nearest match, index %d: "%s" = %4f', [i, matchList.Names[i], match]));
        if (match > bestMatch) then begin
            bestMatch := match;
            returnVal := RecordByFormID(masterFile, StrToInt64(matchList.Names[i]),0);
        end
    end;
    if ((bRefMatchLastPass = 0) and (bestMatch < 0.75)) then begin 
        matchFailed := 1;
    end
    else if (bRefMatchLastPass <> 0) then begin
        if (matchList.count = 1) then begin
            matchFailed := 0;
        end
        else if (bestMatch < 0.6) then begin
            matchFailed := 1;
        end;
    end;

    if (matchFailed = 1) then begin
        addmessage(Format('DEBUG: No acceptable match found for [ %s ]. Closest match was only [ %s ] = %4f out of %d matches.', [IntToHex(GetLoadOrderFormID(e),8), IntToHex(GetLoadOrderFormID(returnVal),8), bestMatch, matchList.count]));
        returnVal := nil;
       	if ((GetIsPersistent(e) <> 0) and (overRideGroupType = 0) and (GetIsPersistent(cell)=0)) then begin
       	    returnVal := FindRefByBestMatch(cell, e, threshold, refMatchRequired, sigMatchRequired, 1);
        end;
        if ((returnVal = nil) and (overRideGroupType = 0)) then begin
            returnVal := FindRefByBestMatch(cell, e, threshold, refMatchRequired, sigMatchRequired, 2);
        end;
    end
    else begin
        addmessage(Format('DEBUG: Exact match not found for [ %s ]. Best match selected is [ %s ] = %4f out of %d matches', [IntToHex(GetLoadOrderFormID(e),8), IntToHex(GetLoadOrderFormID(returnVal),8), bestMatch, matchList.count]));
    end;
...
var
	...
	GridX, GridY, offset, i, threshold, data_flags: integer;
...
//	end;
	...
	if (masterFile <> GetFile(masterCell)) then begin
		...
		data_flags := GetElementNativeValues(targetCell, 'DATA');
		...
		if (data_flags and 1) = 0 then begin // This checks to see if the flag IsInterior=0
            ...
            // Just go ahead and synchronize interior cell now
            SynchronizeInteriorCell(targetCell);
		end;
	end;
    ...
    if (bRefMatchLastPass <> 0) then threshold := 5;
	masterReference := FindRefByBestMatch(masterCell, e, threshold, bRefExactBaseObj, bRefExactBaseClass, 0);
...
begin
    ...
    if ((IsMaster(e) = 0) or (OverrideCount(e) > 0)) then begin
