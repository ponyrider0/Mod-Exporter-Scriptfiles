...
polyreduce_models = False
if ("--polyreduce_models" in argv):
    polyreduce_models = bool(int(argv[argv.index("--polyreduce_models") + 1]))
...
for ob in bpy.data.scenes.active.objects:
    #if (ob.getType() == "Mesh") or (ob.getType() == "Empty"):
    if (True):
